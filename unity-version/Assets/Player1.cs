...
*/
	...
	void Start () {
        ...
        player = new HumanPlayer(5, 5, zone, goal);
    ...
    void FixedUpdate()
    {
        ...
        if (isInZone(tmp))
        {
            ...
            if (t.phase == TouchPhase.Ended)
            {
                ...
                if (distance < 10)
                {
                    GameArbiter.Instance.actionQueue.Enqueue(new Action(swipeStartPosition, swipeEndPosition, intervalTime, Action.ActionType.DEFENSIVE, player));
                    return;
                }

                GameArbiter.Instance.actionQueue.Enqueue(new Action(swipeStartPosition, swipeEndPosition, intervalTime, Action.ActionType.ATTACK, player));
            }
        }
    ...
    {
        DebugDraw.DrawSphere(point, 10, Color.red);
       
        Gizmos.color = Color.yellow;
        Gizmos.DrawSphere(point, 1);
      //  Debug.DrawRay(point, Vector3.up, Color.red, 2, false);
        return player.Zone.transform.collider2D.bounds.Contains(point);
    }
