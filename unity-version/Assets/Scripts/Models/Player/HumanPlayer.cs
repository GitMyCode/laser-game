            ...
            foreach (Touch touch in Input.touches)
            {
                    ...
                    )
                {
                       ...
                       activeTouch == -1)
                    {
                        ...
                        GameArbiter.Instance.actionQueue.Enqueue(new PlayerAction(fingerTrackArray[touch.fingerId],
                            touchPos, intervalTime, PlayerAction.ActionType.ATTACK, gameObject));
                ...
                if (touch.phase == TouchPhase.Ended && isInZone(touchPos))
                {
                       ...
                       activeTouch == -1 && touch.phase == TouchPhase.Ended)
                    {
                        ...
                        swipeCompleteArray[touch.fingerId] = true;
                        GameArbiter.Instance.actionQueue.Enqueue(new PlayerAction(fingerTrackArray[touch.fingerId], touchPos, intervalTime, PlayerAction.ActionType.DEFENSIVE, gameObject));
